...
from scene.portrait3d_mesh_model import Portrait3DMeshGaussianModel
...
class CameraDataset(torch.utils.data.Dataset):
    def __init__(self, cameras: List[Camera], is_val=False):
    ...
    def __getitem__(self, idx):
        if isinstance(idx, int):
            ...
            if not self.is_val:
                ...
                alpha_data = np.array(alpha.convert("L"))
            ...
            # ---- from loadCam() and Camera.__init__() ----
                ...
                camera.original_alpha = resized_alpha
...
class Scene:
    ...
    def __init__(self, args: ModelParams, gaussians,
                 load_iteration=None, shuffle=True, resolution_scales=[1.0], load_ply_path=None):
        """b
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            ...
            print("Found cameras.json file, assuming static Portrait3D data set!")
            scene_info = sceneLoadTypeCallbacks["Portrait3D"](
                args.source_path, args.points_cloud_path, args.images, args.eval)
        else:
        ...
        # create gaussians
        if load_ply_path is not None:
            print("Loading gaussians from point cloud:", load_ply_path)
            self.gaussians.load_ply(load_ply_path,
                                    has_target=args.target_path != "",
                                    )
        elif self.loaded_iter:
            print("Loading gaussians from point cloud:",  os.path.join(self.model_path,
                                                                       "point_cloud",
                                                                       "iteration_" +
            ...
            print("Creating gaussians from point cloud")
    ...
    def save(self, iteration):
        ...
        # teeth_point_cloud_path = os.path.join(
        #     self.model_path, "point_cloud/iteration_{}".format(iteration))
        # self.gaussians.only_save_teeth_ply(os.path.join(
        #     teeth_point_cloud_path, "init_teeth_point_cloud.ply"))

    def getTrainCameras(self, scale=1.0):
        return CameraDataset(self.train_cameras[scale])

    def getValCameras(self, scale=1.0):
        return CameraDataset(self.val_cameras[scale], is_val=True)
