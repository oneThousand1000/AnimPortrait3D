...
def deform_mesh(vertices, 
                deformer,
                lat_rep,
                anchors,
                lat_rep_shape=None):
    
    points_neutral = vertices.float().unsqueeze(0).to(lat_rep.device)
    ...
    with torch.no_grad():
        ...
        for split_id, points in enumerate(grid_points_split):
            ...
            if anchors is not None:
                d, _ = deformer(points, glob_cond, anchors.unsqueeze(
                    1).repeat(1, points.shape[1], 1, 1))
            else:
