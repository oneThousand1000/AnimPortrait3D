...
class GaussianCameraPoseSampler:
    ...
    def sample(horizontal_mean, vertical_mean, horizontal_stddev=0, vertical_stddev=0, radius=1, batch_size=1, device='cpu'):
        ...
        camera_origins[:, 0:1] = radius * \
            torch.sin(phi) * torch.cos(math.pi-theta)
        camera_origins[:, 2:3] = radius * \
            torch.sin(phi) * torch.sin(math.pi-theta)
...
def LookAtPoseCameras(yaws, pitches, lookat_position, radius=1, batch_size=1, device='cpu'):
    batch_size = len(yaws)
    h = torch.tensor(yaws, device=device).view(-1, 1)

    v = torch.tensor(pitches, device=device).view(-1, 1)
...
class UniformCameraPoseSampler:
    ...
    @staticmethod
    def sample(horizontal_mean, vertical_mean, horizontal_stddev=0, vertical_stddev=0, radius=1, batch_size=1, device='cpu'):
        h = (torch.rand((batch_size, 1), device=device) * 2 - 1) * \
            horizontal_stddev + horizontal_mean
        v = (torch.rand((batch_size, 1), device=device)
             * 2 - 1) * vertical_stddev + vertical_mean
        v = torch.clamp(v, 1e-5, math.pi - 1e-5)
