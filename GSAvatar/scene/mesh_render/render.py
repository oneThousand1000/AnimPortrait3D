...
def shade(
    ...
    if 'kd_ks_normal' in material:
        ...
        kd = all_tex[..., :3]
        kd_grad = torch.sum(
            torch.abs(all_tex_jitter[..., :3] - all_tex[..., :3]), dim=-1, keepdim=True) / 3
        ...
        kd_grad = torch.sum(
            torch.abs(kd_jitter[..., 0:3] - kd[..., 0:3]), dim=-1, keepdim=True) / 3
    ...
    if bsdf == 'pbr':
        ...
        if isinstance(lgt, light.EnvironmentLight):
            shaded_col = lgt.shade(gb_pos, gb_normal, kd,
                                   ks, view_pos, specular=True)
        else:
...
def render_layer(
    ...
    if spp > 1 and msaa:
        rast_out_s = util.scale_img_nhwc(
            rast, resolution, mag='nearest', min='nearest')
        rast_out_deriv_s = util.scale_img_nhwc(
            rast_deriv, resolution, mag='nearest', min='nearest') * spp
    else:
        rast_out_s = rast
    ...
    gb_pos, _ = interpolate(
        mesh.v_pos[None, ...], rast_out_s, mesh.t_pos_idx.int())
    ...
    face_normal_indices = (torch.arange(
        0, face_normals.shape[0], dtype=torch.int64, device='cuda')[:, None]).repeat(1, 3)
    ...
    gb_normal, _ = interpolate(
        mesh.v_nrm[None, ...], rast_out_s, mesh.t_nrm_idx.int())
    
    if bsdf !='kd' and bsdf !='ks':
        assert mesh.v_tng is not None, "Tangent space must be provided for {} BSDF".format(bsdf)
        gb_tangent, _ = interpolate(
            mesh.v_tng[None, ...], rast_out_s, mesh.t_tng_idx.int())  # Interpolate tangents
    else:
        gb_tangent = None

    if 'kd_ks_normal' not in mesh.material:
        assert mesh.v_tex is not None, "Texture coordinates must be provided when using texture"
        gb_texc, gb_texc_deriv = interpolate(
            mesh.v_tex[None, ...], rast_out_s, mesh.t_tex_idx.int(), rast_db=rast_out_deriv_s)
    else:
        gb_texc = None
        gb_texc_deriv = None
...
def render_mesh(
    ...
    def composite_buffer(key, layers, background, antialias):
        ...
        for buffers, rast in reversed(layers):
            ...
            if antialias:
                accum = dr.antialias(accum.contiguous(),
                                     rast, v_pos_clip, mesh.t_pos_idx.int())
        return accum
    ...
    mtx_in = torch.tensor(mtx_in, dtype=torch.float32,
                          device='cuda') if not torch.is_tensor(mtx_in) else mtx_in
...
# ==============================================================================================
#  UV padding from https://github.com/Gorilla-Lab-SCUT/Fantasia3D
# ==============================================================================================
...
def uv_padding(image, hole_mask, padding=2, uv_padding_block=4):
    ...
    for (i, j) in tqdm(prods):
        ...
        inpaint_image[i * chunk: (i + 1) * chunk, j *
                      chunk: (j + 1) * chunk] = patch
...
def render_mlp_uv(ctx, mesh, resolution, mlp_texture, uv_padding_block=4):
    # t_tex_idx: faces
    # v_tex: uvs
    # t_pos_idx: faces
    ...
    uv_clip4 = torch.cat((uv_clip, torch.zeros_like(
        uv_clip[..., 0:1]), torch.ones_like(uv_clip[..., 0:1])), dim=-1)

    # rasterize
    rast, _ = dr.rasterize(
        ctx, uv_clip4, mesh.t_tex_idx.int(), resolution)
    hole_mask = ~(rast[..., 3:] > 0)
    tmp_mask = torch.cat([hole_mask, hole_mask, hole_mask], dim=-1)

    # Interpolate world space position
    gb_pos, _ = interpolate(mesh.v_pos[None, ...], rast, mesh.t_pos_idx.int())

    # Sample out textures from MLP
    all_tex = mlp_texture.sample(gb_pos)[..., :3]
    all_tex[tmp_mask] = 1.0

    # ======================================================
    # UV padding from https://github.com/Gorilla-Lab-SCUT/Fantasia3D
    # ======================================================
    # assert all_tex.shape[-1] == 9 or all_tex.shape[-1] == 10, "Combined kd_ks_normal must be 9 or 10 channels"
    print(f'UV padding for Kd...')
    kd = uv_padding(all_tex, hole_mask, uv_padding_block)

    if False:
        '''
        Debug, showing the UV without padding
        '''
        # fill the hole with white color
        hole_mask = hole_mask[0]
        hole_mask = torch.cat([hole_mask, hole_mask, hole_mask], dim=-1)
        kd[hole_mask] = 1.0

    return (rast[..., -1:] > 0).float(), kd


def render_2d_texture_uv(ctx, mesh, resolution, texture_map, uv_padding_block=4):
    # clip space transform
    uv_clip = mesh.v_tex[None, ...]*2.0 - 1.0
    ...
    # rasterize
    rast, rast_deriv = dr.rasterize(
        ctx, uv_clip4, mesh.t_tex_idx.int(), resolution)
    hole_mask = ~(rast[..., 3:] > 0)

    # Interpolate world space position
    gb_pos, _ = interpolate(
        mesh.v_pos[None, ...], rast, mesh.t_pos_idx.int())
    ...
    all_tex = texture_map.sample(gb_texc, gb_texc_deriv)
