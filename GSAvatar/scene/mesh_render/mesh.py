...
def load_mesh(filename,custom_scale=1.0, mtl_override=None):
    name, ext = os.path.splitext(filename)
    if ext == ".obj":
        return unit_size(obj.load_obj(filename, clear_ks=True, mtl_override=mtl_override),custom_scale)
...
######################################################################################
def unit_size(mesh,custom_scale):
    with torch.no_grad():
        vmin, vmax = aabb(mesh)
        scale = 2 / torch.max(vmax - vmin).item()
        v_pos = mesh.v_pos - (vmax + vmin) / 2 # Center mesh on origin
        v_pos = v_pos * scale * custom_scale   # Rescale to unit size
