...
class OrbitCamera:
    ...
    def reset(self):
        ...
        self.rot = R.from_matrix(
            [[1, 0, 0], [0, 1, 0], [0, 0, 1]])  # OpenGL convention
        self.look_at = np.array(
            [0, -0.1, 0], dtype=np.float32)  # look at this point
    ...
    def projection_matrix(self):
        projection_matrix_ = projection_from_intrinsics(self.intrinsics[None], (
            self.image_height, self.image_width), self.znear, self.zfar, z_sign=self.z_sign)[0]
    ...
    def pan(self, dx, dy, dz=0):
        ...
        self.look_at += 2 * self.rot.as_matrix()[:3, :3] @ d * self.radius / \
            self.image_height * math.tan(np.radians(self.fovy) / 2)
