...
class NVDiffRenderer(torch.nn.Module):
    ...
    def world_to_camera(self, vtx, RT):
        ...
        if vtx.shape[-1] == 3:
            posw = torch.cat(
                [vtx, torch.ones([*vtx.shape[:2], 1]).cuda()], axis=-1)
        elif vtx.shape[-1] == 4:
            posw = vtx
        else:
            raise ValueError(
                f"Expected 3D or 4D points but got: {vtx.shape[-1]}")
    ...
    def world_to_clip(self, vtx, RT, K, image_size, mvp=None):
        ...
        mvp = torch.from_numpy(mvp).cuda() if isinstance(
            mvp, np.ndarray) else mvp
        # (x,y,z) -> (x',y',z',w)
        posw = torch.cat([vtx, torch.ones([*vtx.shape[:2], 1]).cuda()], axis=-1)
        # print(posw.shape, mvp.shape)
    ...
    def compute_v_normals(self, verts, faces):
        ...
        v_normals = torch.where(V.dot(v_normals, v_normals) > 1e-20, v_normals,
    ...
    def shade(self, normal, lighting_coeff=None):
        if self.lighting_type == 'constant':
            ...
            diffuse = torch.clamp(V.dot(normal, torch.tensor(
                [0.0, 0.0, 1.0], dtype=torch.float32, device='cuda')), 0.0, 1.0)
        else:
            raise NotImplementedError(
                f"Unknown lighting type: {self.lighting_type}")
        return diffuse

    def render_from_camera(
        self, verts, faces, cam, background_color=[1., 1., 1.],black_normal_bg = True,verts_seg=None, albedo=None
    ):
        """
        ...
        full_proj_transform = cam.full_proj_transform.clone().squeeze()
        ...
        if self.use_opengl:
            ...
            return self.render_mesh(verts, faces, RT, full_proj, 
                                    image_size, background_color,
                                    black_normal_bg = black_normal_bg,
                                    verts_seg=verts_seg, 
                                    albedo=albedo
                                    )
        else:
            if cam.image_height > 2048 or cam.image_width > 2048:
                ...
                image_size = int(cam.image_height // 8 *
                                 8), int(cam.image_width // 8 * 8)

            output = self.render_mesh(
                verts, faces, RT, full_proj, image_size, 
                background_color,
                black_normal_bg=black_normal_bg,
                verts_seg=verts_seg,
                albedo=albedo )
            for k, v in output.items():
                output[k] = F.interpolate(v.permute(
                    0, 3, 1, 2), (cam.image_height, cam.image_width), mode='bilinear').permute(0, 2, 3, 1)
            return output
    ...
    def render_mesh(
        self, verts, faces, RT, full_proj, image_size, background_color=[
            1., 1., 1.], black_normal_bg=True, verts_seg=None,albedo=None
    ):
        """
        Renders meshes into RGBA images
        """
        ...
        verts_clip = self.world_to_clip(
            verts, None, None, image_size, mvp=full_proj)
        tri = faces.int()
        rast_out, rast_out_db = dr.rasterize(
            self.glctx, verts_clip, tri, image_size)
        ...
        #     verts_camera, faces)  # (B, F, 3)
        # face_normals_ = face_normals[:, None, None, :,
        #                              :].expand(-1, W, H, -1, -1)  # (B, 1, 1, F, 3)
        # # (B, W, H, 1, 1)
        # face_id_ = face_id[:, :, :, None].expand(-1, -1, -1, -1, 3)
        # normal = torch.gather(
        #     face_normals_, -2, face_id_).squeeze(-2)  # (B, W, H, 3)
        ...
        depth = verts_camera[..., 2:3].repeat(1, 1,  3) # (B, V, 3)
        # to 0-1
        depth = (depth - depth.max()) / (depth.min() - depth.max())
        depth_img, _ = dr.interpolate(depth, rast_out, faces) 

        if verts_seg is not None:
            segment,_ = dr.interpolate(verts_seg, rast_out, faces)
        ...
        if isinstance(background_color, list):
            ...
            rgba_bg = torch.tensor(
                background_color + [0]).to(rgba).expand_as(rgba)  # RGBA
        elif isinstance(background_color, torch.Tensor):
            """Background as a image"""
            rgba_bg = background_color
            rgba_bg = torch.cat(
                [rgba_bg, torch.zeros_like(rgba_bg[..., :1])], dim=-1)  # RGBA
        else:
            raise ValueError(
                f"Unknown background type: {type(background_color)}")
        rgba_bg = rgba_bg.flip(1)  # opengl camera has y-axis up, needs flipping

        if black_normal_bg:
            black_bg = torch.ones_like(rgba_bg)*-1
        
        else:
            black_bg =torch.ones_like(rgba_bg) 
        normal = torch.where(fg_mask, normal, black_bg[..., :3])
        depth_img = torch.where(fg_mask, depth_img, black_bg[..., :3])
        if verts_seg is not None:
            segment = torch.where(fg_mask, segment,  (torch.ones_like(rgba_bg)*-1)[..., :3])
        ...
        res =  {
            ...
            'rgba': rgba_aa.flip(1),
            'depth': depth_img.flip(1),
            
        }

        if verts_seg is not None:
            res['segment'] = segment.flip(1)
