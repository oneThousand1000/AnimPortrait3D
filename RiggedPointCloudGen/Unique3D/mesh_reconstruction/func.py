...
def laplacian(
    ...
    # adjacency matrix,
    idx = torch.cat([edges, edges.fliplr()], dim=0).type(
        torch.long).T  # (2, 2*E)
...
def _projection(r, device, l=None, t=None, b=None, n=1.0, f=50.0, flip_y=True):
    ...
    p[0, 0] = 2*n/(r-l)  # 1/r
    p[0, 2] = (r+l)/(r-l)  # 0
    p[1, 1] = 2*n/(t-b) * (-1 if flip_y else 1)  # 1/r
    p[1, 2] = (t+b)/(t-b)  # 0
    p[2, 2] = -(f+n)/(f-n)  # -(f+n)/(f-n)
    p[2, 3] = -(2*f*n)/(f-n)
    p[3, 2] = -1
    return p  # 4,4
...
def _orthographic(r, device, l=None, t=None, b=None, n=1.0, f=50.0, flip_y=True):
    ...
    o[0, 0] = 2/(r-l)  # 1/r
    o[0, 3] = -(r+l)/(r-l)  # 0
    o[1, 1] = 2/(t-b) * (-1 if flip_y else 1)  # 1/r
    o[1, 3] = -(t+b)/(t-b)  # 0
    o[2, 2] = -2/(f-n)  # -2/(f-n)
    o[2, 3] = -(f+n)/(f-n)  # -(f+n)/(f-n)
    o[3, 3] = 1
    return o  # 4,4
...
def make_star_cameras(az_count, pol_count, distance: float = 10., r=None, image_size=[512, 512], device='cuda'):
    ...
    theta_rot = torch.eye(3, device=device)[
        None, None].expand(1, P, 3, 3).clone()
...
def make_star_cameras_orthographic(az_count, pol_count, distance: float = 10., r=None, image_size=[512, 512], device='cuda'):
    mv, _ = make_star_cameras(
        az_count, pol_count, distance, r, image_size, device)
    if r is None:
        r = 1
...
def make_sphere(level: int = 2, radius=1., device='cuda') -> Tuple[torch.Tensor, torch.Tensor]:
    sphere = trimesh.creation.icosphere(
        subdivisions=level, radius=1.0, color=None)
    vertices = torch.tensor(sphere.vertices, device=device,
                            dtype=torch.float32) * radius
...
def get_camera(R, T, focal_length=1 / (2**0.5)):
    ...
    camera = FoVOrthographicCameras(device=R.device, R=R, T=T, min_x=-focal_length,
                                    max_x=focal_length, min_y=-focal_length, max_y=focal_length)
    return camera
