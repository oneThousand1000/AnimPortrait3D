...
def get_camera(world_to_cam, fov_in_degrees=60, focal_length=1 / (2**0.5), cam_type='fov'):
    ...
    if cam_type == 'fov':
        ...
        camera = FoVOrthographicCameras(device=world_to_cam.device, R=R, T=T, min_x=-
                                        focal_length, max_x=focal_length, min_y=-focal_length, max_y=focal_length)
    return camera
...
def _warmup(glctx, device=None):
    ...
    pos = tensor([[[-0.8, -0.8, 0, 1], [0.8, -0.8, 0, 1],
                 [-0.8, 0.8, 0, 1]]], dtype=torch.float32)
...
class Pix2FacesRenderer:
    ...
    def transform_vertices(self, meshes: Meshes, cameras: CamerasBase):
        ...
        if z_clip:
            vertices = vertices[vertices[..., 2] >=
                                cameras.get_znear()][None]    # clip
        vertices = vertices * torch.tensor([-1, -1, 1]).to(vertices)
        vertices = torch.cat([vertices, torch.ones_like(
            vertices[..., :1])], dim=-1).to(torch.float32)
        return vertices
    ...
    def render_pix2faces_nvdiff(self, meshes: Meshes, cameras: CamerasBase, H=512, W=512):
        ...
        rast_out, _ = dr.rasterize(
            self._glctx, vertices, faces, resolution=(H, W), grad_db=False)  # C,H,W,4
...
def project_color(meshes: Meshes, cameras: CamerasBase, pil_image: Image.Image,
                  use_alpha=True, eps=0.05, resolution=1024, device="cuda") -> dict:
    """
    ...
    image = torch.from_numpy(np.array(pil_image.convert(
        "RGBA")) / 255.).permute((2, 0, 1)).float().to(device)     # in CHW format of [0, 1.]
    ...
    world_points = cameras.unproject_points(torch.tensor(
        [[[0., 0., 0.1], [0., 0., 0.2]]]).to(device))[0]
    view_direction = world_points[1] - world_points[0]
    view_direction = view_direction / \
        view_direction.norm(dim=0, keepdim=True)  # [3]
    ...
    # print(
    #     f"the number of selected_faces is {len(selected_faces)} , the numer of unique_faces is {len(unique_faces)}")
    ...
    # compute color
    pt_tensor = cameras.transform_points(
        verts_coordinates)[..., :2]  # NDC space points
    valid = ~((pt_tensor.isnan() | (pt_tensor < -1) |
              (1 < pt_tensor)).any(dim=1))  # checked, correct
    valid_pt = pt_tensor[valid, :]
    valid_idx = verts[valid]
    valid_color = torch.nn.functional.grid_sample(image[None].flip((-1, -2)), valid_pt[None, :, None, :], align_corners=False,
    ...
    old_colors[valid_idx] = valid_color * \
        alpha + old_colors[valid_idx] * (1 - alpha)
...
def multiview_color_projection(meshes: Meshes, image_list: List[Image.Image], cameras_list: List[CamerasBase] = None, camera_focal: float = 2 / 1.35, weights=None, eps=0.05, resolution=1024, device="cuda", reweight_with_cosangle="square", use_alpha=True, confidence_threshold=0.1, complete_unseen=False, below_confidence_strategy="smooth") -> Meshes:
    ...
    if cameras_list is None:
        if len(image_list) == 8:
            ...
            raise ValueError(
                "cameras_list is None, and can not be guessed from image_list")
    if weights is None:
        if len(image_list) == 8:
            ...
            raise ValueError(
                "weights is None, and can not be guessed from image_list")
    ...
    for camera, image, weight in zip(cameras_list, image_list, weights):
        ret = project_color(meshes, camera, image, eps=eps,
                            resolution=resolution, device=device, use_alpha=use_alpha)
    ...
    if complete_unseen:
        meshes = complete_unseen_vertex_color(meshes, torch.arange(
            texture_values.shape[0]).to(device)[texture_counts[:, 0] >= confidence_threshold])
    ret_mesh = meshes.detach()
    del meshes
    return ret_mesh


def rotate_mesh(meshes: Meshes, sign) -> Meshes:
    vertex = meshes.verts_list()[0]
    faces = meshes.faces_list()[0]
    textures = meshes.textures.verts_features_packed()

    # rotate vertex 180 degree around y axis
    vertex = torch.matmul(vertex, torch.tensor(  # [V, 3]
        [[-1, 0, 0], [0, 1, 0], [0, 0, -1]]).float().to(vertex.device))

    camera_lookat_point = torch.tensor(
        [[0, 0.01, 0]], device=vertex.device)  # [1, 3]
    vertex = vertex + sign * camera_lookat_point

    mesh = Meshes(verts=[vertex], faces=[faces],
                  textures=TexturesVertex(verts_features=[textures]))
    return mesh


def multiview_color_projection_given_mesh(meshes: Meshes, image_list: List[Image.Image], cameras_list: List[CamerasBase] = None, camera_focal: float = 2 / 1.35, weights=None, eps=0.05, resolution=1024, device="cuda", reweight_with_cosangle="square", use_alpha=True, confidence_threshold=0.1, complete_unseen=False, below_confidence_strategy="smooth") -> Meshes:
    """
    Projects color from a given image onto a 3D mesh.
...
def get_cameras_list(azim_list, device, focal=2/1.35, dist=1.1):
    ...
    for azim in azim_list:
        ...
        cameras: OrthographicCameras = get_camera(
            w2c, focal_length=focal, cam_type='orthogonal').to(device)
        ret.append(cameras)
    return ret
...
def align_with_alpha_bbox(source_img, target_img, final_size=1024):
    ...
    bbox_source_min, bbox_source_max = np.argwhere(source_alpha > 0).min(
        axis=0), np.argwhere(source_alpha > 0).max(axis=0)
    bbox_target_min, bbox_target_max = np.argwhere(target_alpha > 0).min(
        axis=0), np.argwhere(target_alpha > 0).max(axis=0)
    source_content = source_np[bbox_source_min[0]:bbox_source_max[0] +
                               1, bbox_source_min[1]:bbox_source_max[1]+1, :]
    # resize source_content to fit in the position of target_content
    source_content = Image.fromarray(source_content).resize(
        (bbox_target_max[1]-bbox_target_min[1]+1, bbox_target_max[0]-bbox_target_min[0]+1), resample=Image.BICUBIC)
...
def load_image_list_from_img_grid(img_grid_path, resolution=1024):
    ...
    for row in range(0, h, resolution):
        for col in range(0, w, resolution):
            img_list.append(
                grid.crop((col, row, col + resolution, row + resolution)))
    return img_list
