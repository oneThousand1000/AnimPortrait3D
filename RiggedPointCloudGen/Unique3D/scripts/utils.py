    ...
    if mesh.visual is not None:
        if hasattr(mesh.visual, 'vertex_colors'):
            colors = torch.from_numpy(
                mesh.visual.vertex_colors)[..., :3].T / 255.
    if colors is None:
...
def do_rotate(rgba_normal, angle):
    ...
    rotated_normal_tensor = rotated_normal_tensor * \
        rgba_normal[:, :, [3]]    # make bg black
    rgba_normal_np = torch.cat(
        [rotated_normal_tensor * 255, rgba_normal[:, :, [3]] * 255], dim=-1).cpu().numpy()
...
def simple_preprocess(input_image, rembg_session=session, background_color=255):
    ...
    if input_image.mode != 'RGBA':
        ...
        input_image = remove(image_rem, alpha_matting=False,
    ...
    input_image = expand2square(
        input_image, (background_color, background_color, background_color, 0))
    return input_image
...
def init_target(img_pils, new_bkgd=(0., 0., 0.), device="cuda"):
    ...
    img_nps = torch.where(alpha_nps.unsqueeze(-1) > 0.05, ori_img_nps *
                          alpha_nps.unsqueeze(-1) + new_bkgd * (1 - alpha_nps.unsqueeze(-1)), new_bkgd)
