...
if __name__ == '__main__':
    ...
    matte_dir = os.path.join(output_dir, 'mask_images')
    os.makedirs(matte_dir, exist_ok=True)

    print(os.path.join(image_dir,'*.png'))
    for im_path in tqdm(glob.glob(os.path.join(image_dir,'*.png'))):
        # print('Processing:', im_path)

        matte_path = os.path.join(matte_dir, os.path.basename(im_path))
        ...
        white_bg_img = np.array(Image.open(im_path)) * ( matting_array[:, :, None] / 255) + 255 *  (1 - matting_array[:, :, None] / 255)  
        white_bg_img_path = os.path.join(output_dir, os.path.basename(im_path))
        white_bg_img = white_bg_img.astype(np.uint8)
        Image.fromarray(white_bg_img).save(white_bg_img_path)
