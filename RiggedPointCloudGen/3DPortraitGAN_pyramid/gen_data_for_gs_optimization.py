...
@click.option('--yaw_num', type=int, help='yaw angles sample num', default=8)
@click.option('--pitch_num', type=int, help='pitch key angles num', default=8)
def generate_images(
    network_pkl: str,
    output_dir: str,
    ckpt_path: str,
    ...
    original_img_dir = os.path.join(output_dir, 'original_images')
    ...
    pitchs = [int(60+60/(pitch_num-1)*i)
              for i in range(pitch_num)]  # 90-30 to 90+30
    yaws = [int(90 + 360/yaw_num * i) % 360 for i in range(yaw_num)]
    print(pitchs)
    print(yaws)
    pbar = tqdm(total=yaw_num * pitch_num)
    for angle_p in pitchs:
        ...
        for angle_y in yaws:
            ...
            mask = output['image_mask']
            # whilte_bg_img = img * mask + 1 * (1 - mask)

            img = (img.permute(0, 2, 3, 1) * 127.5 +
                   128).clamp(0, 255).to(torch.uint8)
            PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(
                f'{original_img_dir}/{sample_idx:04d}.png')

            # mask = (mask.permute(0, 2, 3, 1) *
            #         255).clamp(0, 255).to(torch.uint8)
            # mask = torch.cat([mask, mask, mask], dim=-1)
            # PIL.Image.fromarray(mask[0].cpu().numpy(), 'RGB').save(
            #     f'{mask_img_dir}/{sample_idx:04d}.png')

            # whilte_bg_img = (whilte_bg_img.permute(0, 2, 3, 1) * 127.5 +
            #                  128).clamp(0, 255).to(torch.uint8)
            # PIL.Image.fromarray(whilte_bg_img[0].cpu().numpy(), 'RGB').save(
            #     f'{output_dir}/{sample_idx:04d}.png')

            # bg = (bg.permute(0, 2, 3, 1) * 127.5 +
            #       128).clamp(0, 255).to(torch.uint8)
            # PIL.Image.fromarray(bg[0].cpu().numpy(), 'RGB').save(
            #     f'{output_dir}/background.png')

            # if render_normal:
            #     normal = (normal.permute(0, 2, 3, 1) * 127.5 +
            #               128).clamp(0, 255).to(torch.uint8)
            #     PIL.Image.fromarray(normal[0].cpu().numpy(), 'RGB').save(
            #         f'{output_dir}/{sample_idx:04d}_rendered_normal.png')
            ...
            #     f'{output_dir}/{sample_idx:04d}_rendered_depth.png')
            ...
            camera_info[f'{sample_idx:04d}.png'] = {
    ...
    #                            os.path.join(mesh_dir, f'original_shape.ply'), level=15)
