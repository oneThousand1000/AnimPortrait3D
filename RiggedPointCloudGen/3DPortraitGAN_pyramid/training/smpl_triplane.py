...
class TriPlaneGenerator(torch.nn.Module):
    def __init__(self,
        ...
        print('ended bcg_synthesis_kwargs')
        ...
        self.renderer = ImportanceRenderer(
            w_dim=w_dim, num_ws=14, batch_size=self.batch_size, thickness=thickness, box_warp=rendering_kwargs['box_warp'])
        ...
        self.backbone = StyleGAN2Backbone(z_dim, c_dim + 6, w_dim, img_resolution=256,
                                          img_channels=self.trigrid_channel * 3 *
                                          rendering_kwargs['triplane_depth'],
        ...
        self.superresolution = dnnlib.util.construct_class_by_name(
            class_name=rendering_kwargs['superresolution_module'], channels=self.decode_channel, img_resolution=img_resolution, sr_num_fp16_res=sr_num_fp16_res, sr_antialias=rendering_kwargs['sr_antialias'], **sr_kwargs)
        ...
        self.torgb = ToRGBLayer(self.decode_channel, 3, w_dim) if rendering_kwargs.get(
        ...
        # planes = self.backbone.synthesis(torch.zeros(
        #     4, self.backbone.synthesis.num_ws, w_dim), update_emas=False, **synthesis_kwargs)
    ...
    def synthesis(self, ws, c, neural_rendering_resolution=None, update_emas=False, cache_backbone=False, use_cached_backbone=False,
        ...
        if self.bcg_synthesis:
            ws_bcg = ws[:, :self.bcg_synthesis.num_ws] if ws_bcg is None else ws_bcg[:,
                                                                                     :self.bcg_synthesis.num_ws]
            if ws_bcg.size(1) < self.bcg_synthesis.num_ws:
                ws_bcg = torch.cat(
                    [ws_bcg, ws_bcg[:, -1:].repeat(1, self.bcg_synthesis.num_ws-ws_bcg.size(1), 1)], 1)
        ...
        if self.torgb:
            ...
            bcg_rgb_image = self.torgb(
                bcg_image, ws_bcg[:, -1], fused_modconv=False)
            bcg_rgb_image = bcg_rgb_image.to(
                dtype=torch.float32, memory_format=torch.contiguous_format)
        else:
    ...
    def render_planes(self, ws, planes, c, neural_rendering_resolution=None, update_emas=False, chunk=None, render_bg=True,
                      patch_resolution=None, input_patch_info=None,
                      apply_def=False, pose_params=None, ws_bcg=None, render_normal=False,
                      **synthesis_kwargs):
        cam2world_matrix = c[:, :16].view(-1, 4, 4)
        intrinsics = c[:, 16:25].view(-1, 3, 3)
        ...
        patch_info = []
        if patch_resolution is None or patch_resolution == neural_rendering_resolution:
            # Create a batch of rays for volume rendering
            ray_origins, ray_directions = self.ray_sampler(
                cam2world_matrix, intrinsics, neural_rendering_resolution)
            ...
            ray_origins, ray_directions, patch_info = self.ray_sampler.patch_forward(cam2world_matrix, intrinsics,
                                                                                     patch_resolution, input_patch_info=input_patch_info,
        ...
        if chunk is not None:
            ...
            weights_samples = render_output['weights']
            if render_normal:
                normal_samples = render_output['normals_final']
        ...
        depth_image = depth_samples.permute(0, 2, 1).reshape(N, 1, H, W)
        weights_samples = weights_samples.permute(0, 2, 1).reshape(N, 1, H, W)
        normal_samples = normal_samples.permute(0, 2, 1).reshape(
            N, 3, H, W).contiguous() if render_normal else None
        ...
        out = {'image_raw': rgb_image, 'image_depth': depth_image, "image_mask": mask_image,
               'patch_info': patch_info, 'ray_directions': ray_directions}
        if render_normal:
            out['image_normal'] = normal_samples
    ...
    def sample(self, coordinates, directions, z, c, p, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        # Compute RGB features, density for arbitrary 3D coordinates. Mostly used for extracting shapes.
        ws = self.mapping(z, c, p, truncation_psi=truncation_psi,
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, neural_rendering_resolution=None, update_emas=False, cache_backbone=False, use_cached_backbone=False,
        ...
        ws = self.mapping(z, c, pose_params, truncation_psi=truncation_psi,
                          truncation_cutoff=truncation_cutoff, update_emas=update_emas)
        # TODO
