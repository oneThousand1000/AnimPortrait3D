...
class TriPlaneGenerator(torch.nn.Module):
    ...
    def render_planes(self, ws, planes, c, neural_rendering_resolution=None, update_emas=False, chunk=None, render_bg=True,
                      ...
                      apply_def=False, pose_params=None, ws_bcg=None, render_normal=False, global_scale = 1.0, 
                      **synthesis_kwargs):
        ...
        if chunk is not None:
            ...
            for _ro, _rd in zip(torch.split(ray_origins, chunk, dim=1), torch.split(ray_directions, chunk, dim=1)):
                render_output = self.renderer(planes, self.decoder, _ro,
                                              ...
                                              global_scale = global_scale)  # channels last
            ...
            render_output = self.renderer(planes, self.decoder, ray_origins,
                                          ...
                                          pose_params=pose_params, 
                                          render_normal=render_normal,
                                          global_scale = global_scale,triplane_crop=triplane_crop)  # channels last
        ...
        out = {'image_raw': rgb_image, 'image_depth': depth_image,
               "image_mask": mask_image, 'patch_info': patch_info,
               'ray_directions': ray_directions, "image_background": bcg_rgb_image}
        if render_normal:
