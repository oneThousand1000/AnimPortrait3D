...
def remesh(
    ...
    face_collapse = calc_face_collapses(vertices, faces, edges, face_to_edge,
                                        edge_length, face_normals, vertex_normals, min_edgelen, area_ratio=0.5)
    # e[0,1] 0...ok, 1...edgelen=0
    shortness = (1 - edge_length /
                 min_edgelen[edges].mean(dim=-1)).clamp_min_(0)
...
class MeshOptimizer:
    ...
    def step(self):
        ...
        torch_scatter.scatter_mean(src=edge_smooth.flip(dims=[1]).reshape(
            E*2, -1), index=edges.reshape(E*2, 1), dim=0, out=neighbor_smooth)
        ...
        grad = torch.addcmul(self._vertices.grad, laplace,
                             self._nu[:, None], value=self._laplacian_weight)
        ...
        # update vertices
        ramped_lr = self._lr * \
            min(1, self._step * (1-self._betas[0]) / self._ramp)
    ...
    def remesh(self, flip: bool = True, poisson=False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        self._vertices_etc, self._faces = remesh(
            self._vertices_etc, self._faces, min_edge_len, max_edge_len, flip, max_vertices=1e6)
