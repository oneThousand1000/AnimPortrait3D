...
class LandmarkDetectorSTAR:
    ...
    def detect_dataset(self, dataloader):
        ...
        for item in tqdm(dataloader): 
            ...
            logger.info(
                f"Annotate facial landmarks for image {name}..."
            )
            ...
            if len(bbox) == 0:
                logger.error(
                    f"No bbox found for image {name}. Setting landmarks to all -1."
                )  
    ...
    def annotate_landmarks(self, dataloader):
        ...
        for image_name, lmk_face_camera in lmks_face.items():
            bounding_box = bboxes_faces[image_name]
            face_landmark_2d = lmk_face_camera
            ...
            out_path =  dataloader.dataset.cfg.root_folder /  "landmark2d/STAR/{}.npz".format(image_name.replace(".png", "")) 
