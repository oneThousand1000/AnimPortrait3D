...
class FlameTracker:
    ...
    def load_from_tracked_flame_params(self, fp):
        ...
        load_param_list(self.scale, report["scale"])
    ...
    def compute_energy(
        ...
        """
        Compute total energy for frame  
        :param sample: 
        ...
        if stage is not None:
            _log_dict = self.compute_regularization_energy(
                result_dict, verts, verts_cano, lmks, albedos, 0, include_keyframes, stage
            )
    ...
    def evaluate(self, make_visualization=True, epoch=0):
        ...
        texture_path = self.out_dir / f'fitted_flame_mesh_{epoch}.jpg'
        mtl_path = self.out_dir / f'fitted_flame_mesh_{epoch}.mtl'
        obj_path = self.out_dir / f'fitted_flame_mesh_{epoch}.obj'
...
class SingleFrameTracker(FlameTracker):
    def __init__(self, cfg: BaseTrackingConfig):
        super().__init__(cfg)
    ...
    def detect_landmarks(self, cfg):
        ...
        if cfg.data.landmark_source in ['star', 'both']:
            ...
            if not cfg.exp.reuse_landmarks or not (dataset.cfg.root_folder /  "landmark2d/STAR").exists():
                # LandmarkDetector only supports a batch_size of 1
                dataloader = DataLoader(dataset, batch_size=1, shuffle=False, num_workers=4)
    ...
    def init_params(self):
        ...
        # rigid pose
        self.translation = torch.tensor([0.0152, 0.4206, 0.2139]).expand(self.n_timesteps, 3).to(self.device)
        self.scale =  torch.tensor([2.7783]).expand(self.n_timesteps, 1).to(self.device)
        self.rotation =  torch.tensor([0.0946, -0.0293,  0.0161]).expand(self.n_timesteps, 3).to(self.device)
        ...
        train_tensors += (
            [self.shape, self.translation,self.scale, self.rotation, self.neck_pose, self.jaw_pose, self.eyes_pose, self.expr,]
        )
    ...
    def optimize_stage(
        ...
        print(f"Optimization of stage {stage} done.")
    ...
    def get_train_parameters(
        ...
        if self.opt_dict["pose"]:
            ...
            params["scale"].append(self.scale)
